function criaGui() {
  ...
  folder
    ...
    .onFinishChange(function() {
      if (safeAltura != params.altura) {
        ...
        meshDireita.geometry = new THREE.BoxGeometry(
          largura,
          altura,
          espessura
        );
        meshDireita.position.y = altura / 2 + espessura;
        meshEsquerda.geometry = new THREE.BoxGeometry(
          largura,
          altura,
          espessura
        );
        ...
        meshAtras.geometry = new THREE.BoxGeometry(
          comprimento,
          altura,
          espessura
        );
        meshAtras.position.y = altura / 2 + espessura;
        maxDivisaoHorizontal =
          Math.floor(params.altura / valorMinimoDivisaoHorizontal) - 1;
        nDivisaoHorizontalFolder.max(maxDivisaoHorizontal);
        for (var i = 0; i < listMeshDivisao.length; i++) {
          listMeshDivisao[i].geometry = new THREE.BoxGeometry(
            largura,
            altura,
            espessura
          );
  ...
  folder
    ...
    .onFinishChange(function() {
      if (safeComprimento != params.comprimento) {
        ...
        meshCima.geometry = new THREE.BoxGeometry(
          comprimento,
          largura,
          espessura
        );
        meshBaixo.geometry = new THREE.BoxGeometry(
          comprimento,
          largura,
          espessura
        );
        ...
        for (var j = 0; j < listMeshDivisaoHorizontal.length; j++) {
          listMeshDivisaoHorizontal[j].geometry = new THREE.BoxGeometry(
    ...
    .onFinishChange(function() {
      if (safeLargura != params.largura) {
        ...
        for (var j = 0; j < listMeshDivisaoHorizontal.length; j++) {
          ...
          listMeshDivisaoHorizontal[j].position.z =
            -largura / 2 + espessura / 2;
        }
      }
  ...
  //Num Divisoes
  var nDivisaoFolder = folder
    .add(params, "nDivisao")
    .name("Nº Div Verticais")
    .min(0)
    .max(maxDivisao)
    .onFinishChange(function() {
  ...
  var nDivisaoHorizontalFolder = folder
    .add(params, "nDivisaoHorizontal")
    .name("Nº Div Horizontais")
    .min(0)
    .max(maxDivisaoHorizontal)
    .onFinishChange(function() {
  ...
  folder1.add(selects, "selectLimparTudo").name("Remover Partes");
  folder.open();
  var ficheiroTextura;
  ...
  folderAglomerado
    .add(selectsTextura, "selectAglomerado")
    .name("Natural").domElement.previousSibling.style.backgroundImage =
    "url(" + texturaMadeiraCompressada + ")";
  folderAglomerado
    .add(selectsTextura, "selectAglomeradoVerniz")
    .name("Envernizado").domElement.previousSibling.style.backgroundImage =
    "url(" + texturaMadeiraDetalhada + ")";

  folderMadeira
    .add(selectsTextura, "selectMadeira")
    .name("Natural").domElement.previousSibling.style.backgroundImage =
    "url(" + texturaMadeiraClara + ")";
  folderMadeira
    .add(selectsTextura, "selectMadeiraEnvernizado")
    .name("Envernizado").domElement.previousSibling.style.backgroundImage =
    "url(" + texturaMadeiraEscura + ")";

  folderMetal
    .add(selectsTextura, "selectMetal")
    .name("Sem Acabamento").domElement.previousSibling.style.backgroundImage =
    "url(" + texturaMetal + ")";
...
function changeTexturaInterior(ficheiroTextura) {
  for (var mesh in listaSelectedPartes) {
    listaSelectedPartes[mesh].material.map = new THREE.TextureLoader().load(
      ficheiroTextura
    );
  }

  for (var mesh in listMeshDivisao) {
    listMeshDivisao[mesh].material.map = new THREE.TextureLoader().load(
      ficheiroTextura
    );
  }
}
